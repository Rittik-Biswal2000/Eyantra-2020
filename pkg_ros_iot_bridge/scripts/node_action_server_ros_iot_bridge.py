    ...
    def process_goal(self, goal_handle):
        ...
        if(goal.protocol == "mqtt"):
            ...
            if(goal.mode == "pub"):
                ...
                x=goal.message[1]
                y=goal.message[4]
                theta=goal.message[7]

                if goal.message[1]== '-':
                    x=-1*int(goal.message[2])
                if goal.message[4]== '-':
                    y=-1*int(goal.message[5])
                if goal.message[7]== '-':
                    theta=-1*int(goal.message[8])    

                # Used to send the data(x,y,theta) to our own spreadsheet and to the web url provided in the md book
                send_sheet={"id":"Sheet1","turtle_x":x,"turtle_y":y,"turtle_theta":theta}
                ...
                parameters = {"id":"task1", "team_id":"vb_0566", "unique_id":"AiRiAkAk", "turtle_x":x, "turtle_y":y, "turtle_theta":theta} 
...
def main():
    ...
    print("")
    print("")
    print("To pass data to ros iot bridge node , mqtt client should publish on this mqtt topic - eyrc/AiRiAkAk/iot_to_ros")
    print("To get data from ros iot bridge node , mqtt client should subscribe on this mqtt topic - eyrc/AiRiAkAk/ros_to_iot")
    print("")
    print("")
    print("Publish [start] on [eyrc/AiRiAkAk/iot_to_ros] to start the turtle")
    print("")
    print("")
